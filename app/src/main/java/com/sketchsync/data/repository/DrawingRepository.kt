...
class DrawingRepository @Inject constructor(
     ...
     * 设置光标断开时自动清理
     */
    fun setCursorOnDisconnect(roomId: String, userId: String) {
        roomsRef.child(roomId).child("cursors").child(userId).onDisconnect().removeValue()
    }

    /**
     * 主动移除用户光标
     */
    fun removeCursor(roomId: String, userId: String) {
        roomsRef.child(roomId).child("cursors").child(userId).removeValue()
    }
